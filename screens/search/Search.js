...
import { View, Text, Dimensions, StyleSheet, TouchableOpacity, FlatList, Image, TextInput } from 'react-native';
...
getListProduct = async ()=>{
  ...
  var response = await axios({
    url: `https://mainf-app.herokuapp.com/api/product/searchByKeyword?keyword=${this.state.keyword}`,
    headers: {
  ...
  return (
      ...
      <View style={styles.wrapperH}>
              ...
              <TextInput 
                  ...
                  autoCorrect={false}
                  onChangeText={text => {this.setState({ keyword: text }),this.submitKey()}}
                  value={this.state.keyword}
              />
          </View>
      <View style={{flex:9, height:width/3, justifyContent:'center', margin: 10, backgroundColor: '#fff',}}>
