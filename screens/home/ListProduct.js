...
export default class ListProduct extends Component {
  ...
  getListProduct = async () => {
    var response = await axios({
      url: `https://mainf-app.herokuapp.com/api/product/getProductByType?productType=${this.props.navigation.getParam(
        'idType',
      )}`,
      headers: {
          ...
          <View style={styles.container}>
            <View style={styles.header}>
              <TouchableOpacity onPress={() => this.props.navigation.goBack()}>
                <Ionicons name={'ios-arrow-back'} size={25} color={'#faaca8'} />
              </TouchableOpacity>
              <View />
            </View>
            <FlatList
